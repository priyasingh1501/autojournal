...
import { clearImageCache, backfillCachedImagesToSupabase } from '../services/WisdomImageService';
    ...
    <SafeAreaView style={styles.container} edges={['bottom']}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity
            style={[styles.uploadBtn, backfilling && { opacity: 0.5 }]}
            disabled={backfilling}
            onPress={async () => {
              setBackfilling(true);
              setBackfillProgress({ done: 0, total: 0 });
              try {
                const result = await backfillCachedImagesToSupabase((done, total) =>
                  setBackfillProgress({ done, total }),
                );
                Alert.alert(
                  'Upload complete',
                  `Uploaded: ${result.uploaded}\nAlready on server: ${result.skipped}\nFailed: ${result.failed}\nTotal cached: ${result.total}` +
                  (result.firstError ? `\n\nFirst error:\n${result.firstError}` : ''),
                );
              } catch (err: any) {
          ...
          >
            <Feather name="upload-cloud" size={14} color="rgba(152,212,250,0.85)" />
            <Text style={styles.uploadText}>
              {backfilling && backfillProgress
                ? `Uploading ${backfillProgress.done}/${backfillProgress.total}…`
                : 'Upload cached images to cloud'}
            </Text>
          </TouchableOpacity>
